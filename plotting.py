...
def plot_episode_trajectory(
    ...
    # Time-step dots at regular intervals
    ego_dot_indices = np.arange(0, len(ego_arr), dot_every)
    enc_dot_indices = np.arange(0, len(enc_arr), dot_every)
    ...
    # Annotate every dot with its step number
...
def plot_robustness(
    ...
    for step_idx, rob in enumerate(ep_robustness):
        ...
        # rob = (trace_list, lower_time_points, upper_time_points)
        trace_list = rob[0]
    ...
    # Zero line — above zero means encounter possibly/definitely present
